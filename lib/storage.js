...
// "owner/name"; also accepts a pasted clone URL like https://github.com/owner/name.git
const GH_REPO = (process.env.GITHUB_REPO || '').trim()
  .replace(/^(https?:\/\/)?(www\.)?github\.com[/:]/i, '').replace(/^git@github\.com:/i, '')
...
async function gh(method, body) {
  ...
  // GitHub also answers 404 when the token can't see a private repo.
  if (method === 'GET' && res.status === 404) {
    throw new Error(`${GH_FILE} not found in ${GH_REPO}${GH_BRANCH ? ` (${GH_BRANCH})` : ''}, or the token can't access that repo`);
  }
